...
//        requestUrl += (/\?/.test(mqLayer.options.url) ? '&' : '?') + params;
        ...
        var reqObj = wmsgfi.buildWMSOptions(Mapbender.Util.removeProxy(mqLayer.olLayer.url), [mqLayer.olLayer], {x: x, y: y}, mqLayer.olLayer.params.FORMAT);
        reqObj.params['LAYERS'] = reqObj.params['QUERY_LAYERS'] = mqLayer.olLayer.queryLayers;
        reqObj.params['EXCEPTIONS'] = mqLayer.source.configuration.options.exception_format;//,//"application/vnd.ogc.se_xml"
