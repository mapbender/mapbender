(function() {
  ...
  $.widget("ui.dialogExtend", {
    ...
    _initButtons: function() {
      ...
      titlebar = $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar");
    ...
    _initModuleButton: function(name, mode) {
      ...
      buttonPane = $(this.element[0]).popupDialog("widget").find('.ui-dialog-titlebar-buttonpane');
    ...
    _initTitleBar: function() {
      ...
      switch (this.options.titlebar) {
        case false:
          return 0;
        case "none":
          if ($(this.element[0]).popupDialog("option", "draggable")) {
            handle = $("<div />").addClass("ui-dialog-draggable-handle").css("cursor", "move").height(5);
            $(this.element[0]).popupDialog("widget").prepend(handle).draggable("option", "handle", handle);
          }
          return $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar").find(".ui-dialog-title").html("&nbsp;").end().css({
        ...
        case "transparent":
          return $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar").css({
            "background-color": "transparent",
            "background-image": "none",
            "border": 0
          });
    ...
    _restore: function() {
      if (this._state !== "normal") {
        ...
        this._setState("normal");
        return $(this.element[0]).popupDialog("widget").focus();
      }
    },
    _saveSnapshot: function() {
      if (this._state === "normal") {
        this.original_config_resizable = $(this.element[0]).popupDialog("option", "resizable");
        this.original_config_draggable = $(this.element[0]).popupDialog("option", "draggable");
        this.original_size_height = $(this.element[0]).popupDialog("widget").outerHeight();
        this.original_size_width = $(this.element[0]).popupDialog("option", "width");
        this.original_size_maxHeight = $(this.element[0]).popupDialog("option", "maxHeight");
        this.original_position_mode = $(this.element[0]).popupDialog("widget").css("position");
        this.original_position_left = $(this.element[0]).popupDialog("widget").offset().left - $('body').scrollLeft();
        this.original_position_top = $(this.element[0]).popupDialog("widget").offset().top - $('body').scrollTop();
        return this.original_titlebar_wrap = $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar").css("white-space");
      }
    },
    ...
    _toggleButtons: function(newstate) {
      ...
      $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar-restore").toggle(state !== "normal").css({
        "right": "1.4em"
      }).end();
      _ref = this.modes;
      for (name in _ref) {
        mode = _ref[name];
        $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar-" + name).toggle(state !== mode.state && this.options[mode.option]);
      }
      _ref1 = this.modes;
      _results = [];
      for (name in _ref1) {
        mode = _ref1[name];
        if (mode.state === state) {
          _results.push($(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar-restore").insertAfter($(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar-" + name)).end());
        } else {
          _results.push(void 0);
        }
      }
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    collapse: function() {
      ...
      newHeight = $(this.element[0]).popupDialog("widget").find(".ui-dialog-titlebar").height() + 15;
      this._trigger("beforeCollapse");
      if (this._state !== "normal") {
        this._restore();
      }
      this._saveSnapshot();
      pos = $(this.element[0]).popupDialog("widget").position();
      $(this.element[0]).popupDialog("option", {
        "resizable": false,
        "height": newHeight,
        "maxHeight": newHeight,
        "position": [pos.left - $(document).scrollLeft(), pos.top - $(document).scrollTop()]
      }).on('dialogclose', this._collapse_restore).hide().popupDialog("widget").find(".ui-dialog-buttonpane:visible").hide().end().find(".ui-dialog-titlebar").css("white-space", "nowrap").end().find(".ui-dialog-content");
    ...
    _restore_collapsed: function() {
      ...
      return $(this.element[0]).show().popupDialog("widget").find(".ui-dialog-buttonpane:hidden").show().end().find(".ui-dialog-titlebar").css("white-space", original.titlebar.wrap).end().find(".ui-dialog-content").popupDialog("option", {
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    maximize: function() {
      ...
      this._saveSnapshot();
      if ($(this.element[0]).popupDialog("option", "draggable")) {
        $(this.element[0]).popupDialog("widget").draggable("option", "handle", null).find(".ui-dialog-draggable-handle").css("cursor", "text").end();
      }
      $(this.element[0]).popupDialog("widget").css("position", "fixed").find(".ui-dialog-content").show().popupDialog("widget").find(".ui-dialog-buttonpane").show().end().find(".ui-dialog-content").popupDialog("option", {
        "resizable": false,
        "draggable": false,
        "height": newHeight,
        "width": newWidth,
    ...
    _restore_maximized: function() {
      ...
      original = this._loadSnapshot();
      $(this.element[0]).popupDialog("widget").css("position", original.position.mode).find(".ui-dialog-titlebar").css("white-space", original.titlebar.wrap).end().find(".ui-dialog-content").popupDialog("option", {
        "resizable": original.config.resizable,
      ...
      if ($(this.element[0]).popupDialog("option", "draggable")) {
        return $(this.element[0]).popupDialog("widget").draggable("option", "handle", $(this.element[0]).popupDialog("widget").find(".ui-dialog-draggable-handle").length ? $(this.element[0]).popupDialog("widget").find(".ui-dialog-draggable-handle") : ".ui-dialog-titlebar").find(".ui-dialog-draggable-handle").css("cursor", "move");
      }
    },
...
(function() {
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    minimize: function() {
      ...
      dialogcontrols = $(this.element[0]).popupDialog("widget").clone().children().remove().end();
      $(this.element[0]).popupDialog("widget").find('.ui-dialog-titlebar').clone(true, true).appendTo(dialogcontrols);
      dialogcontrols.css({
      ...
      $(this.element[0]).on('dialogbeforeclose', this._minimize_restoreOnClose).popupDialog("widget").hide();
      this._setState("minimized");
      return this._trigger("minimize");
    },
    _restore_minimized: function() {
      $(this.element[0]).popupDialog("widget").show();
