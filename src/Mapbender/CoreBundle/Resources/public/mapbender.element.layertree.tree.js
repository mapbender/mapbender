                ...
                $(that).sortable({
                    ...
                    update: function(event, ui){
                        ...
                        if($(ui.item).attr("data-type") !== self.consts.root) {
                            toMove['layerIdx'] = {id: $(ui.item).attr("data-id")};
                        }
                        /* "before" at layerTree is "after" at model.sourceTree */
                        self.model.changeSource({change: {move: {tomove: toMove, before: after, after: before}}});
                    }
                });
            });
        },
        ...
        _removeSource: function(e){
            ...
            if(layer.sourceid && layer.type) {
                switch(layer.type) {
                    ...
                    case types.group:
                    case types.simple:
                        model.changeSource({change: {layerRemove: {sourceIdx: {id: layer.sourceid}, layer: {options: {
                                            id: layer.id}}}}});
                        break;
                }
            }
