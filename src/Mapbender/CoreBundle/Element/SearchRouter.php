...
class SearchRouter extends AbstractElementService implements ConfigMigrationInterface, ElementHttpHandlerInterface
{
    ...
    public function handleRequest(Element $element, Request $request)
    {
        ...
        if (in_array($action, ['autocomplete', 'search'])) {
            // for first form, the csrf token id generated by Symfony is the FQCN of the form type, for the subsequent
            // forms it's the category id
            $tokenId = $categoryId === "0" ? SearchRouterFormType::class : $categoryId;
            $token = isset($data['properties']['_token']) ? new CsrfToken($tokenId, $data['properties']['_token']) : null;
