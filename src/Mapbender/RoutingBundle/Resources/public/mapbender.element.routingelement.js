(function($) {
    ...
    _create: function() {
        if(!Mapbender.checkTarget("mbRoutingElement", this.options.target)) {
            return;
        }
    ...
    _setup: function() {
        ...
        this.olMap = this.mbMap.map.olMap;

        this.map.on('mousedown', this.mapClickProxy);
            ...
            $(this).autocomplete({
                ...
                source: function (request, response) {
                    self._search(encodeURI(request.term)).then(function(_response) {
                        ...
                        // clear errorDiv
                        errorDiv.empty();
                        // check exits routeError of _response
                        if (_response.routeError) {
                            self._searchErrorHandling(_response.routeError);
                        } else {
                ...
                select: function (event, ui){
                    $(this).val(ui.item.label);
                    var wktString = ui.item.geom,
                        wktOl = new OpenLayers.Format.WKT();
                    var feature = wktOl.read(wktString),
                        srs = ui.item.srid,
                    ...
                    if (requestProj.projCode !== mapProj.projCode) {
                        coords = self._transformCoordinates(
                    ...
                    self._createMarker(this, ll);
                    var targetExtent = self.markerLayer.getDataExtent(),
                        targetZoom = self.olMap.getZoomForExtent(targetExtent),
                        targetCenter = targetExtent.getCenterLonLat();
                    self.olMap.moveTo(targetCenter, targetZoom - 1);
    ...
    _getClickCoordinates: function(event) {
        var x = event.pageX - this.map.offset().left;
        var y = event.pageY - this.map.offset().top;

        return this.olMap.getLonLatFromPixel(new OpenLayers.Pixel(x, y));
    },
    ...
    _mapClickHandler: function(event) {
        var self = this;
        // TODO ugly!!
        this.mapClickHandlerCoordinate = self._getClickCoordinates(event);
    },
    ...
    _ensureMarkerLayer: function() {
        var self = this;
        // if marker layer does not exist, create it
        if (!self.markerLayer) {
            self.markerLayer = new OpenLayers.Layer.Vector('routingMarkerLayer');
            self.olMap.addLayer(self.markerLayer);
            self.mbMap.map.olMap.addLayer(self.markerLayer);
            // Make features draggable
            self.mbMap.map.olMap.addControl(new OpenLayers.Control.DragFeature(self.markerLayer, {
                autoActivate: true,
