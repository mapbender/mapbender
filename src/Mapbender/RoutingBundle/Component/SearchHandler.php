...
class SearchHandler {
    ...
    public function search($requestParams, $configuration): JsonResponse
    {
        ...
        $response = [
            'error' => [
                'message' => 'No search-driver configured.',
            ],
        ];
        ...
        if ($configuration['useSearch'] && !empty($searchConfig)) {
            switch ($driver) {
                case 'solr';
                    $response = $this->solrSearch($requestParams, $searchConfig);
                    break;
                default:
    ...
    protected function solrSearch($requestParams, $searchConfig)
    {
    ...
    protected function createSolrQuery($configuration, $terms) {
        $query = '';
